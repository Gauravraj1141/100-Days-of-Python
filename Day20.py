...
Questions_list = ["Q. who is the president of India ? ",
                  "Q. who is the pm of India ? ",
                  "Q. The red sandalwood or red sanders trees, shown in the film “Pusha: The Rise”, are endemic to which region of India ?",
                  "Q.  What name has been given to the 216-feet statue of Sri Ramanujacharya, unveiled in Hyderabad in February 2022 ? ",
                  ]
Option_list = [
...
rep = 0
while rep < len(Questions_list):
    rep +=1

    try:
        i = random.randint(0,len(Questions_list))
        ...
        if User_Answer == Answer_list[i]:
            ...
            speak_and_print(f"your answer is Wrong and Right answer is : {Answer_list[i]} ")
            speak_and_print(f"you Lose {Price_list[i]} rupees Better luck next time!  ")
            User_Account_Bal = User_Account_Bal - Price_list[i]
            break
